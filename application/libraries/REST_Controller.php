...
abstract class REST_Controller extends MX_Controller
{
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        $object_called = preg_replace('/^(.*)\.(?:' . implode('|', array_keys($this->_supported_formats)) . ')$/', '$1',
            $object_called);
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === false) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => false,
                $this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_invalid_api_key'),
                    $this->rest->key)
            ], self::HTTP_FORBIDDEN);
        }
    ...
    protected function _detect_input_format()
    {
        ...
        if (empty($content_type) === false) {
            ...
            $content_type = (strpos($content_type, ';') !== false ? current(explode(';',
    ...
    protected function _detect_method()
    {
        ...
        return in_array($method, $this->allowed_http_methods) && method_exists($this,
            '_parse_' . $method) ? $method : 'get';
    }
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            if (!($row = $this->rest->db->where($this->config->item('rest_key_column'),
                $key)->get($this->config->item('rest_keys_table'))->row())) {
                return false;
            }
        ...
        if ($username !== null) {
            ...
            if (strpos(strtolower($http_auth), 'basic') === 0) {
                ...
                list($username, $password) = explode(':',
                    base64_decode(substr($this->input->server('HTTP_AUTHORIZATION'), 6)));
            }
        }
